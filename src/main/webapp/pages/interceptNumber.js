...
$(function(){
	...
	grid = $("#maingrid").ligerGrid({
		...
		url : basePath + "/findInterceptNumberPage.do",
		parms:[{
			name : "number",
			value : param.number
			}
...
function save(){
	...
	if(validate(param)){
		$.ajax({
			type:"post",//请求方式
	 		url:basePath + "/addInterceptNumber.do",//发送请求地址
	 		data:param,
	 	 	//请求成功后的回调函数有两个参数
...
function update(){
	...
	if(validate(param)){
		$.ajax({
			type:"post",//请求方式
	 		url:basePath + "/updateInterceptNumber.do",//发送请求地址
	 		data:param,
	 	 	//请求成功后的回调函数有两个参数
	 	 	success:function(data,textStatus){
			   if(data==""){
	...
	if (rows.length > 0) {
		$.ligerDialog.confirm('删除后数据无法恢复，确定删除？', function(yes) {
			if (yes) {
				$.ajax({
					type : "post",// 请求方式
					url : basePath + "/deleteInterceptNumber.do",// 发送请求地址
					data : {// 发送给数据库的数据
						json : $.toJSON(rows)
					},
					// 请求成功后的回调函数有两个参数
